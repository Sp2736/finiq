...
const MobileUserCard = ({ member }: { member: any }) => {
  ...
  return (
    <div 
      ...
      className={`bg-white p-4 rounded-2xl border cursor-pointer ${isExpanded ? 'border-emerald-200 shadow-md ring-1 ring-emerald-50' : 'border-slate-200 shadow-sm'} relative overflow-hidden transition-all group`}
    >
      ...
      <div className={`grid transition-all duration-300 ease-in-out ${isExpanded ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0 mt-0'}`}>
        <div className="overflow-hidden">
          <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3.5 rounded-xl ml-2">
          ...
          <div className="flex justify-end gap-2 mt-3 ml-2">
             <button className="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-50 transition-colors">Edit</button>
             <button className="px-4 py-2 bg-emerald-50 text-emerald-700 rounded-lg text-xs font-bold hover:bg-emerald-100 transition-colors">View Details</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="h-full flex flex-col relative z-10 animate-[fadeIn_0.5s_ease-out] overflow-hidden">
      ...
      <div className="shrink-0 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6 md:mb-8">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <button className="hidden md:flex items-center gap-2 px-5 py-2.5 bg-emerald-600 text-white rounded-xl font-medium text-sm hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/20">
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        ...
        <div className="hidden md:flex flex-col flex-1 min-h-0 bg-white rounded-xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
