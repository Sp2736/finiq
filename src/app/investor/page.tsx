...
import CalculatorsModules from "./CalculatorsModules";
...
export default function UnifiedPortfolioApp() {
  ...
  const normalizedPortfolio = useMemo((): ClientPortfolio | null => {
    ...
    return {
      ...
      totalCapital:
        d.total_capital || d.invested_capital || d.totalCapital || 0,
  ...
  const [mobileActiveScreen, setMobileActiveScreen] = useState<
    "holdings" | "fund_details"
  >("holdings");
  const [mobileSelectedFund, setMobileSelectedFund] =
    useState<UnifiedFund | null>(null);
  ...
  const handleOpenAnalytics = (fund: UnifiedFund) => {
    if (fund.amfiCode) {
      ...
      prefetchQueue.current = prefetchQueue.current.filter(
        (c) => c !== fund.amfiCode,
      );
    }
    ...
    if (normalizedPortfolio?.funds) {
      normalizedPortfolio.funds.forEach((fund: any) => {
        if (activeFilterType === "Category/Industry")
          options.add(fund.category);
        if (activeFilterType === "AMC/Issuer") options.add(fund.amc);
        if (activeFilterType === "Tag" && fund.statusTag)
          options.add(fund.statusTag);
      });
    }
  ...
  const filteredFunds = useMemo(() => {
    ...
    return normalizedPortfolio.funds.filter((fund: any) => {
      if (activeFilterType === "All" || activeFilterValue === "All")
        return true;
      if (activeFilterType === "Category/Industry")
        return fund.category === activeFilterValue;
      if (activeFilterType === "AMC/Issuer")
        return fund.amc === activeFilterValue;
      if (activeFilterType === "Tag")
        return fund.statusTag === activeFilterValue;
      return true;
    });
