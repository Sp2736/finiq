...
export default function OTPVerificationForm({ onVerify, onBack, phoneInfo }: OTPVerificationFormProps) {
  ...
  // Updated default focus colors to match the emerald theme
  const getInputBorderColor = () => {
    ...
    return 'border-slate-200 text-slate-900 bg-white focus:bg-white focus:border-emerald-500 focus:ring-emerald-500/20';
  };
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <div className="w-full relative">
        ...
        {/* Updated button to use emerald styling instead of indigo */}
        <button
          onClick={() => triggerVerification(otp.join(''))}
          disabled={otp.some(char => char === '') || status === 'loading' || status === 'success'}
          className="w-full py-2.5 md:py-4 px-4 mt-2 md:mt-4 bg-emerald-600 text-white text-sm md:text-base font-bold rounded-md shadow-md hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
        >
          {status === 'loading' ? (
