...
export default function GlobalStatsRibbon({ client, useCompactValues = false }: GlobalStatsRibbonProps) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-xl rounded-xl border border-slate-200/60 shadow-sm overflow-x-auto custom-scrollbar">
      <div className="flex w-max min-w-full divide-x divide-slate-100 p-1.5">
        {[
          ...
          { label: "Total Returns", value: getFormattedValue(client.unrealisedGain), sub: `(${formatPercent(client.unrealisedGainPercent)})`, valNum: client.unrealisedGain },
          ...
          // ─── FIX: whitespace-nowrap and min-w-max enforce single-line layout ───
          <div key={idx} className="flex-1 px-4 py-2.5 min-w-max whitespace-nowrap" title={stat.title}>
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              {stat.sub && (
                <span className={`text-[10px] font-bold tabular-nums ${stat.valNum !== undefined ? getStatusColor(stat.valNum) : 'text-slate-500'}`}>
                  {stat.sub}
                </span>
              )}
            </div>
