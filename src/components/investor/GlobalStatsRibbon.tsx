        ...
        {[
          ...
          <div key={idx} className="flex-1 px-6 py-4 min-w-[160px]">
            ...
            <div className="flex items-baseline gap-2">
              <p className={`text-xl font-black tabular-nums tracking-tight ${stat.highlight ? 'text-primary' : (stat.valNum !== undefined ? getStatusColor(stat.valNum) : 'text-slate-900')}`}>
                {stat.value}
              </p>
              {stat.sub && (
                <span className={`text-xs font-bold ${stat.valNum !== undefined ? getStatusColor(stat.valNum) : ''}`}>{stat.sub}</span>
              )}
            </div>
          </div>
        ))}
